...
const SPEAKER_COLORS = ["#388bfd","#f78166","#ffa657","#7ee787","#d2a8ff","#79c0ff","#56d364","#ff7b72"];

const speakerColor = (sp) => {
  if (!sp) return "#8b949e";
  return SPEAKER_COLORS[sp.charCodeAt(sp.length - 1) % SPEAKER_COLORS.length];
};
...
export default function App() {
  ...
  const saveKey = () => {
    ...
    if (!k) return;
    localStorage.setItem("aai_key", k);
    setApiKey(k);
  ...
  const handleFile = (f) => {
    ...
    if (!ok) { alert("Please upload an audio or video file."); return; }
    setFile(f);
  ...
  format_text: false,
      ...
      if (!res.ok) {
        const d = await res.json().catch(() => ({}));
        throw new Error(d.error || `Job submit failed (${res.status}) — check API key`);
      }
      const data = await res.json();
  ...
  const buildSegments = (t) => {
    ...
    const highlights = new Set(
      (t.auto_highlights_result?.results || [])
        .flatMap((h) => h.text.toLowerCase().split(" "))
    );
    return t.utterances.map((u, i) => {
      ...
      const emotion =
        Object.entries(dominant).sort((a, b) => b[1] - a[1])[0]?.[0] || "NEUTRAL";
      const accent = t.language_code
        ? t.language_code.replace("_", "-").toUpperCase()
        : "en-US";
      return {
        id: i,
  ...
  const exportJSON = () => {
    if (!result) return;
    const blob = new Blob([JSON.stringify(result.segments, null, 2)], {
      type: "application/json",
    });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = `annotation_${file?.name || "output"}.json`;
    a.click();
  ...
  const seekTo = (secs) => {
    if (mediaRef.current) mediaRef.current.currentTime = parseFloat(secs);
  };
  ...
  return (
    <div style={{ fontFamily: "'Courier New', monospace", background: "#0a0c10", minHeight: "100vh", color: "#cdd9e5" }}>

      {/* Header */}
      <div style={{ background: "#0d1117", borderBottom: "1px solid #21262d", padding: "14px 18px 10px", position: "sticky", top: 0, zIndex: 100 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
          ...
          <div style={{ marginLeft: "auto", display: "flex", gap: 8, alignItems: "center" }}>
            {result && (
              <button onClick={exportJSON} style={sBtn(exportMsg ? "#2ea043" : "#388bfd")}>
                {exportMsg || "⬇ JSON"}
              </button>
            )}
            <button onClick={() => setShowKeyForm((v) => !v)} style={sBtn("#6e7681")}>🔑 API Key</button>
          </div>
        </div>
        ...
        {showKeyForm && (
          <div style={{ marginTop: 10, display: "flex", gap: 8 }}>
            <input
              ...
              style={{ flex: 1, background: "#161b22", border: "1px solid #30363d", borderRadius: 6, color: "#e6edf3", padding: "7px 10px", fontSize: 11, fontFamily: "inherit", outline: "none" }}
            />
            <button onClick={saveKey} style={sBtn("#2ea043")}>Save</button>
            <a href="https://www.assemblyai.com" target="_blank" rel="noreferrer" style={{ fontSize: 10, color: "#388bfd", alignSelf: "center" }}>Get free →</a>
          </div>
        )}
      </div>
      ...
      <div style={{ padding: "18px 16px", maxWidth: 820, margin: "0 auto" }}>
        ...
        {!apiKey && (
          <div style={{ background: "#2d1b00", border: "1px solid #bb8009", borderRadius: 10, padding: "12px 14px", marginBottom: 14, fontSize: 12, color: "#e3b341" }}>
            ⚠ Needs a free <strong>AssemblyAI API key</strong>.{" "}
            <a href="https://www.assemblyai.com" target="_blank" rel="noreferrer" style={{ color: "#388bfd" }}>Get one free at assemblyai.com</a>
            {" "}→ tap 🔑 API Key above to enter it.
          </div>
        )}
        ...
        {status !== "done" && (
          <div
            ...
            style={{
              ...
              background: file ? "#0d21190d" : "#161b22",
        ...
        {fileURL && status !== "done" && (
          <audio ref={mediaRef} src={fileURL} controls style={{ width: "100%", marginBottom: 14, borderRadius: 8 }} />
        )}
        ...
        {(status === "uploading" || status === "processing") && (
          <div style={{ background: "#161b22", border: "1px solid #21262d", borderRadius: 12, padding: "28px", textAlign: "center", marginBottom: 18 }}>
            <div style={{ fontSize: 30, marginBottom: 10 }}>⚙️</div>
            <div style={{ fontSize: 13, color: "#f0f6fc", fontWeight: 700, marginBottom: 6 }}>{progress}</div>
            <div style={{ fontSize: 11, color: "#6e7681", marginBottom: 14 }}>AI is detecting speakers, emotions, emphasis, overlaps...</div>
        ...
        {status === "error" && (
          <div style={{ background: "#2d1014", border: "1px solid #da3633", borderRadius: 10, padding: "14px", marginBottom: 14 }}>
            <div style={{ fontSize: 12, color: "#f85149", fontWeight: 700, marginBottom: 4 }}>⚠ Error</div>
            <div style={{ fontSize: 11, color: "#f85149", marginBottom: 10 }}>{errorMsg}</div>
            <button onClick={() => setStatus("idle")} style={sBtn("#da3633")}>Try Again</button>
          </div>
        )}
        ...
        {status === "done" && result && (
          <div>
            {/* Summary */}
            <div style={{ background: "#161b22", border: "1px solid #21262d", borderRadius: 12, padding: "14px 16px", marginBottom: 12, display: "flex", gap: 20, flexWrap: "wrap" }}>
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 12 }}>
              {speakers.map((sp) => (
                <div key={sp} style={{ background: speakerColor(sp) + "22", border: `1px solid ${speakerColor(sp)}`, borderRadius: 6, padding: "3px 10px", fontSize: 10, color: speakerColor(sp), fontWeight: 700 }}>
                  {sp}
                </div>
              ))}
            </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {filteredSegments.map((seg) => (
                <SegCard key={seg.id} seg={seg} onSeek={seekTo} mediaRef={mediaRef} />
              ))}
            </div>

            <button
              onClick={() => { setStatus("idle"); setResult(null); setFile(null); setFileURL(null); }}
              style={{ width: "100%", marginTop: 18, background: "transparent", border: "1px solid #30363d", borderRadius: 8, color: "#8b949e", padding: "12px", fontSize: 12, cursor: "pointer", fontFamily: "inherit" }}
...
function SegCard({ seg, onSeek, mediaRef }) {
  ...
  const playSegment = () => {
    ...
    setTimeout(() => mediaRef.current && mediaRef.current.pause(), dur);
  };

  return (
    <div style={{ background: "#161b22", border: `1px solid ${seg.overlaps ? "#bb8009" : "#21262d"}`, borderLeft: `4px solid ${color}`, borderRadius: 10, padding: "12px 14px" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8, flexWrap: "wrap" }}>
        <span style={{ background: color + "22", color, padding: "2px 10px", borderRadius: 4, fontSize: 11, fontWeight: 800 }}>{seg.speaker}</span>
        <span style={{ fontSize: 10, color: "#8b949e", cursor: "pointer" }} onClick={() => onSeek(seg.startTime)}>
      ...
      <div style={{ fontSize: 13, color: "#e6edf3", lineHeight: 1.9, marginBottom: 8 }}>
        {seg.words.length > 0
          ? seg.words.map((w, i) => {
              const em = seg.emphasizedWords.includes(w.text);
              return (
                <span key={i} onClick={() => onSeek(w.start / 1000)} title={`${(w.start / 1000).toFixed(2)}s`}
                  style={{ color: em ? "#ffa657" : "#e6edf3", fontWeight: em ? 800 : 400, textDecoration: em ? "underline" : "none", cursor: "pointer" }}>
                  {w.text}{" "}
                </span>
              );
            })
      ...
      <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
        ...
        <button onClick={() => setExpanded((v) => !v)} style={{ marginLeft: "auto", background: "transparent", border: "none", color: "#6e7681", fontSize: 10, cursor: "pointer", fontFamily: "inherit" }}>
      ...
      {expanded && (
        <div style={{ marginTop: 10, paddingTop: 10, borderTop: "1px solid #21262d", display: "flex", gap: 16, flexWrap: "wrap" }}>
          <Det label="Confidence" value={`${Math.round((seg.confidence || 0) * 100)}%`} />
          <Det label="Locale" value={seg.accent} />
          <Det label="Duration" value={`${seg.duration}s`} />
          <Det label="Overlap" value={seg.overlaps ? "Yes ↔" : "No"} />
          <Det label="Words" value={seg.words.length} />
        </div>
      )}
    </div>
  );
...
function Badge({ color, text }) {
  return (
    <span style={{ background: color + "22", border: `1px solid ${color}`, borderRadius: 4, padding: "1px 7px", fontSize: 9, color }}>{text}</span>
  );
}
